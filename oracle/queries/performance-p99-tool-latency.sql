...
SELECT
  ROUND(PERCENTILE_CONT(total_ms, 0.50) OVER (), 6) AS p50_tool_latency,
  ROUND(PERCENTILE_CONT(total_ms, 0.75) OVER (), 6) AS p75_tool_latency,
  ROUND(PERCENTILE_CONT(total_ms, 0.90) OVER (), 6) AS p90_tool_latency,
  ROUND(PERCENTILE_CONT(total_ms, 0.99) OVER (), 6) AS p99_tool_latency
FROM (
